...
Robot::Robot() : robotId(-1), position({0.0, 0.0}), batteryLevel(100.0), 
                 currentTask(0), maxSpeed(1.6), loadCapacity(100), isAvailable(true) {}

Robot::Robot(int id, pair<double, double> pos, double battery, int taskId, 
...
void Robot::setCurrentTask(const int& task) { currentTask = task; }
