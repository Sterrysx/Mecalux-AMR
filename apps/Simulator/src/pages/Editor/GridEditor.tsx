...
export default function GridEditor() {
  ...
  return (
    <div 
      className={`mx-auto max-w-[1100px] p-6 space-y-4 ${darkMode ? 'text-slate-100' : 'text-slate-900'}`} 
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className={`text-2xl font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>
            {distribution.meta.name} <span className={`${darkMode ? 'text-slate-300' : 'text-slate-500'}`}>({W}×{H})</span>
          </h1>
          <p className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
            cell size: {distribution.meta.cell_size_m} m
          </p>
        </div>
        <button 
          onClick={exportJSON} 
        ...
        {([["PATH","g","#3db86b"],["OBSTACLE","#","#9ca3af"],["RESTRICTED","X","#7b1113"],["CHARGER","C","#1e90ff"],["PACKET","P","#8b5a2b"],["EMPTY",".","#ffffff"]] as [BrushKey,string,string][])
          ...
          <button 
            ...
            className={`flex items-center gap-2 rounded-md border px-3 py-1 text-sm ${
              brush===k
                ? darkMode ? "border-slate-300 bg-slate-700" : "border-slate-900 bg-slate-100"
                : darkMode ? "border-slate-600 hover:border-slate-500 hover:bg-slate-700" : "border-slate-300 hover:border-slate-400"
            }`}
          >
        ...
        <svg width={svgW} height={svgH} className="block">
          {grid.map((row,y)=>row.map((ch,x)=>(
            <rect key={`${x}-${y}`} x={x*CELL} y={(H-1-y)*CELL} width={CELL} height={CELL}
              fill={CELL_COLORS[ch]??"#eee"} stroke={darkMode ? "#475569" : "#cbd5e1"} strokeWidth={1}
